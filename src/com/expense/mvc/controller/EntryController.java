...
public class EntryController {
	private static final Logger logger = Logger.getLogger("log." + EntryController.class);
	...
	public String acbalance(Model model, HttpSession session) {
		...
		try {
			return JsonUtils.toJson(new AjaxBean(Errors.OK, "", loginService.getAllActiveAccounts(loginService
					.getDataKey(session))));
		} catch (Exception e) {
	...
	public String acbalance(@PathVariable int acctId, Model model, HttpSession session) {
		...
		try {
			return JsonUtils.toJson(new AjaxBean(Errors.OK, "", loginService.getAccountUI(
					loginService.getDataKey(session), acctId)));
		} catch (Exception e) {
			EntryController.logger.error("Account acbalance failed.. :: ", e);
			return JsonUtils.toJson(new AjaxBean(Errors.PROCESS, ""));
		}
