...
public class UploadService {
	private static final Logger logger = Logger.getLogger("log." + UploadService.class);
	...
	private List<TransactionUI> buildUIs(String data) throws IOException, ParseException {
		...
		CSVParser parser = new CSVParser(new StringReader(data));
		String[] fields = parser.getLine();

		/*
		 * #0.AdjustInd (Y) #1.TransDt (mm/dd/yyyy) #2.Description #3.CategoryID #4.FromAccID
		 * #5.ToAccID #6.AdhocInd (Y) #7.Amount
		 */
		...
		while (fields != null) {
			...
			// #0 AdjustInd
			boolean adjust = BooleanUtils.toBoolean(fields[0].trim(), "Y", "");
			if (adjust) {
			...
			// #1 TransDt
			ui.setTransDate(DateUtils.parseDate(fields[1].trim(), "MM/dd/yyyy"));
			ui.setTransMonth(DateUtils.truncate(ui.getTransDate(), Calendar.MONTH));

			// #2 Description
			ui.setDescription(fields[2].trim());

			// #3 Category ID
			if (StringUtils.isNotBlank(fields[3])) {
				ui.setCategoryId(Integer.parseInt(fields[3].trim()));
			}

			// #4 From Acct ID
			ui.setFromAccountId(Integer.parseInt(fields[4].trim()));

			// #5 To Acct ID
			if (StringUtils.isNotBlank(fields[5])) {
				ui.setToAccountId(Integer.parseInt(fields[5].trim()));
			}

			// #6 AdhocInd
			boolean adhoc = BooleanUtils.toBoolean(fields[6].trim(), "Y", "");
			if (adhoc) {
			...
			// #7 Amount
			ui.setAmount(Double.parseDouble(fields[7].trim()));
